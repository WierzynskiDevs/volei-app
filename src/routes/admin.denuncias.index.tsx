...
import { REPORT_STATUS_LABEL, reports, type ReportStatus, type ReportTarget } from "@/lib/admin-data";

export const Route = createFileRoute("/admin/denuncias/")({
  head: () => ({
    meta: [
      { title: "Central de denúncias · Super Admin BeacHub" },
      { name: "description", content: "Triagem e resolução de denúncias contra organizadores, participantes, arenas e eventos na plataforma BeacHub." },
...
const targets: ("TODOS" | ReportTarget)[] = ["TODOS", "ORGANIZADOR", "PARTICIPANTE", "ARENA", "EVENTO"];
  ...
  return (
    <AdminShell>
      <div className="mx-auto max-w-7xl px-4 py-8">
          ...
          {targets.map((t) => (
            <button
              ...
              className={`border px-3 py-1.5 font-display text-[11px] font-bold uppercase tracking-widest ${
                target === t ? "border-graphite bg-graphite text-background" : "border-border text-muted-foreground"
              }`}
            >
              {t}
            </button>
        ...
        <AdminTable head={["Protocolo", "Alvo", "Denunciado", "Motivo", "Data", "Prioridade", "Status", ""]}>
          {rows.map((r) => (
            <tr key={r.id}>
              <td className="px-4 py-3 font-display text-sm font-bold">{r.code}</td>
              <td className="px-4 py-3 text-xs uppercase tracking-wide text-muted-foreground">{r.target}</td>
              ...
              <td className="px-4 py-3">
                <StatusPill tone={r.priority === "Alta" ? "danger" : r.priority === "Média" ? "warn" : "neutral"}>
                  {r.priority}
                </StatusPill>
              </td>
              <td className="px-4 py-3">
