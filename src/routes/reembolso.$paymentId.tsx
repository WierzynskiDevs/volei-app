...
import { brl, eventChanges, getPayment, refunds, REFUND_CAUSE_LABEL } from "@/lib/finance-data";
...
export const Route = createFileRoute("/reembolso/$paymentId")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) {
      return {
        meta: [
          { title: "Reembolso indisponível · BeacHub" },
    ...
    const description =
      "Solicite e acompanhe o reembolso da sua inscrição, com status confirmado pelo gateway.";
    return {
      meta: [
        { title },
...
function RefundPage() {
  ...
  const [cause, setCause] = useState<"EVENTO_ALTERADO" | "DESISTENCIA">(
    change ? "EVENTO_ALTERADO" : "DESISTENCIA",
  );
      ...
      <div className="mx-auto max-w-2xl px-4 py-8">
        ...
        <div className="mt-4 divide-y divide-border border border-border bg-card">
          <MoneyRow
            ...
            detail={`${payment.code} · ${payment.gatewayId}`}
            strong
          />
          ...
          <section className="mt-6 border border-border bg-card">
            <div className="flex flex-wrap items-center gap-3 border-b border-border px-4 py-3">
              ...
              <span className="text-sm text-muted-foreground">
                {REFUND_CAUSE_LABEL[existing.cause]}
              </span>
          ...
          <div className="mt-6 border border-warning/40 bg-warning/10 p-6">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              ...
              {cause === "EVENTO_ALTERADO"
                ? "Não posso participar após a alteração do evento."
                : "Desistência do participante."}{" "}
              Valor: {brl(payment.amountCents)}. O reembolso só é considerado concluído após
          ...
          <>
            {change ? (
              <FinanceAlert tone="warn" title="Este evento sofreu alteração">
                {change.fields.map((f) => `${f.label}: ${f.from} → ${f.to}`).join(" · ")} — Motivo:{" "}
            ...
            {cause === "DESISTENCIA" ? (
              <div className="mt-4 border border-border bg-sand p-4 text-sm text-muted-foreground">
                Você está solicitando cancelamento de uma inscrição em um evento que permanece
                conforme publicado. De acordo com a política do evento, essa solicitação pode não
                gerar reembolso.
              </div>
            ) : null}
