  ...
  head: () => ({
    meta: [
      { title: "Financeiro · Organizador BeacHub" },
      {
        name: "description",
        content:
          "Receita bruta, taxa da plataforma, taxas do Asaas, valor líquido, pendências e reembolsos dos seus campeonatos.",
      },
      { property: "og:title", content: "Financeiro · Organizador BeacHub" },
      {
        property: "og:description",
        content: "Quanto você arrecadou, quanto pagou de taxa e quanto vai receber.",
      },
    ],
  }),
      ...
      <div className="mx-auto max-w-6xl px-4 py-8">
        ...
        <div className="mt-6 space-y-3">
          {data && !data.organizer.can_receive_payments ? (
            <FinanceAlert tone="danger" title="Conta de recebimento não vinculada">
              Sem conta vinculada não é possível publicar evento pago — não há destino para o
              repasse.
            </FinanceAlert>
          ) : null}
          {data && data.pending.count > 0 ? (
            <FinanceAlert
              ...
              action={
                <Link
                  to="/organizador/inscricoes"
                  className="border border-border px-3 py-1.5 font-display text-[10px] font-bold uppercase tracking-widest"
                >
                  Cobrar
                </Link>
              }
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Stat
            ...
            hint={brl(totals?.gross_cents ?? 0)}
          />
          <Stat
            label="Pagamentos pendentes"
            value={data?.pending.count ?? 0}
            hint={brl(data?.pending.gross_cents ?? 0)}
          />
          <MoneyCard
            label="Reembolsos"
          ...
          <MoneyCard
            label="Saldo"
            cents={balance}
            hint="Líquido menos reembolsos e chargebacks"
            emphasis
          />
        </div>
        ...
        <section className="mt-8 border border-border bg-card">
          <div className="border-b border-border px-4 py-3">
            ...
            <p className="text-sm text-muted-foreground">
              O jogador paga apenas o valor da inscrição. A taxa da plataforma e as taxas do Asaas
              são descontadas via split.
            </p>
          </div>
          <div className="divide-y divide-border">
            <MoneyRow
              label="Receita bruta das inscrições"
              cents={totals?.gross_cents ?? 0}
            ...
            <MoneyRow
              label="Taxa da plataforma"
              cents={totals?.platform_fee_cents ?? 0}
              detail="Receita do SaaS · taxa congelada em cada transação"
              negative
            />
            <MoneyRow
              label="Taxas Asaas"
              cents={totals?.asaas_fee_cents ?? 0}
              detail="Custo de processamento financeiro — responsabilidade do organizador"
              negative
            />
            <MoneyRow
              label="Reembolsos confirmados"
              cents={totals?.refunded_cents ?? 0}
              detail="Estornos processados pelo gateway"
              negative
            />
            <MoneyRow
              label="Chargebacks"
              cents={totals?.chargeback_cents ?? 0}
              detail="Contestações de cartão"
              negative
            />
            <MoneyRow
              label="Saldo do organizador"
              cents={balance}
              detail="Valor efetivamente destinado a você"
              strong
            />
          </div>
        </section>
              ...
              <tbody className="divide-y divide-border">
                {isPending ? (
                  ...
                  <tr>
                    ...
                    >
                      Nenhum evento movimentou dinheiro ainda. A primeira linha aparece quando uma
                      inscrição for paga.
                    </td>
                  </tr>
                ) : (
                  (data?.by_event ?? []).map((e) => (
                    <tr key={e.event_id}>
                      ...
                      <td className="score-num px-4 py-3 tabular-nums">
                        {brl(e.organizer_net_cents)}
                      </td>
                      <td className="score-num px-4 py-3 font-bold tabular-nums">
                        {brl(balanceCents(e))}
                      </td>
                    </tr>
                  ))
        ...
        <section id="reembolsos" className="mt-8">
          ...
          <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
            O reembolso só é considerado concluído após confirmação do Asaas — não é possível marcar
            manualmente como reembolsado.
          </p>
          <p className="mt-3 border border-border bg-card px-4 py-8 text-center text-sm text-muted-foreground">
            O fluxo de solicitação de reembolso ainda não está implementado. Estornos já confirmados
            pelo gateway aparecem nos totais acima.
          </p>
        </section>
      </div>
    </AppShell>
