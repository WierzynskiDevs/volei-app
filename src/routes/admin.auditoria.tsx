...
export const Route = createFileRoute("/admin/auditoria")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Registro de todas as ações administrativas da plataforma BeacHub: quem fez, o quê, quando e por quê." },
      ...
      <div className="mx-auto max-w-7xl px-4 py-8">
        ...
        <AdminTable head={["Data e hora", "Responsável", "Ação", "Alvo", "Motivo"]}>
          ...
          >
            {logs.map((a) => (
              <tr key={a.id}>
                <td className="px-4 py-3 text-xs text-muted-foreground">{formatDateTime(a.created_at)}</td>
                {/*
