  ...
  head: () => ({
    meta: [
      { title: "Meus eventos · Organizador BeacHub" },
      {
        name: "description",
        content:
          "Todos os seus campeonatos com status de inscrições, arrecadação e alterações comunicadas.",
      },
      { property: "og:title", content: "Meus eventos · Organizador BeacHub" },
      {
        property: "og:description",
        content: "Operação e arrecadação de cada campeonato em um só lugar.",
      },
    ],
  }),
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-6xl px-4 py-8">
        ...
        {isPending ? (
          <div
            className="mt-6 divide-y divide-border border border-border bg-card"
            aria-busy="true"
          >
            {[0, 1, 2].map((i) => (
        ...
        <p className="mt-4 text-xs text-muted-foreground">
          Alterações de data, horário ou local exigem justificativa e podem gerar obrigação de
          reembolso aos inscritos.
        </p>
        ...
        <div className="mt-8">
          ...
          <div className="mt-3 border border-border bg-card px-4 py-8 text-center text-sm text-muted-foreground">
            Cada alteração de evento publicado é registrada com a sua justificativa. A consulta
            desse histórico pelo organizador ainda não está disponível.
          </div>
        </div>
      </div>
    </AppShell>
