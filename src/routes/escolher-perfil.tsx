  ...
  head: () => ({
    meta: [
      { title: "Como deseja acessar? · BeacHub" },
      {
        name: "description",
        content:
          "Escolha entre a experiência de jogador e a de organizador de campeonatos de vôlei de areia.",
      },
      { property: "og:title", content: "Como deseja acessar? · BeacHub" },
      {
        property: "og:description",
        content: "Seletor de perfil para contas com múltiplos papéis.",
      },
    ],
  }),
      ...
      <div className="mx-auto max-w-xl px-4 py-16">
        ...
        {roles.length < 2 ? (
          <p className="mt-3 text-muted-foreground">
            Esta conta possui apenas um papel — o seletor só aparece para quem tem múltiplos
            acessos.
          </p>
        ) : null}
          ...
          {roles.map((role) => (
            ...
            >
              <span className="text-2xl">
                {role === "ORGANIZER" ? "🏆" : role === "SUPER_ADMIN" ? "🛡️" : "🏐"}
              </span>
              <span className="font-display text-lg font-bold">{ROLE_LABEL[role]}</span>
              <span className="ml-auto font-display text-[10px] font-bold uppercase tracking-widest text-accent">
                Acessar
              </span>
            </button>
          ))}
